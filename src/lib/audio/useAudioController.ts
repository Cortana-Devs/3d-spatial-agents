...
export function useAudioController() {
    ...
    if (typeof window !== "undefined") {
      ...
      if (globalAudioCtx.state === "suspended") {
        try {
          ...
          // Silently fail as AudioUnlocker or user interaction will handle resumption
        }
      }
    }
  ...
  const initFallbackWorker = useCallback(() => {
    if (typeof window !== "undefined" && !globalWorker) {
      globalWorker = new Worker(new URL("./kokoroWorker.js", import.meta.url), {
        type: "module",
      });
  ...
  useEffect(() => {
    ...
    const flushQueue = async () => {
      if (globalHasInteracted) return;
      console.debug(
        `[AudioController] Interaction detected - Flushing ${globalSpeechQueue.length} queued speech tasks.`,
      );
      ...
      for (const task of tasks) {
        setTimeout(
          () => speak(task.text, task.agentId, task.isSubconscious),
          50,
        );
      }
  ...
  const speak = useCallback(
    async (text: string, agentId?: string, isSubconscious = false) => {
      ...
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: [
            {
              parts: [
                { text: `Read aloud in a warm, welcoming tone: ${text}` },
              ],
            },
          ],
          config: {
        ...
        console.warn(
          "[AudioController] Gemini TTS Failed. Falling back to Puter/OpenAI...",
          err,
        );
      }
      ...
      try {
        ...
        console.warn(
          "[AudioController] Puter Cloud Failed. Fallback to Kokoro...",
          err,
        );
      }
