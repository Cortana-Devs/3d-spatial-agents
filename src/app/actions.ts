...
export async function generateAgentThought(context: AgentContext, memoryContext: string, sessionId?: string) {
    ...
    const effectiveSessionId = sessionId || 'unknown-session-' + crypto.randomUUID().slice(0, 8);
    try {
...
export async function generateReflection(textToSummarize: string, sessionId?: string) {
    const requestId = crypto.randomUUID();
    const effectiveSessionId = sessionId || 'unknown-session-' + crypto.randomUUID().slice(0, 8);
    ...
    try {
        ...
        const prompt = "You are an agent's memory manager. Summarize the following events into a single, concise 'Insight' or 'Fact' that captures the key context. Ignore mundane details.";
