...
class NavigationNetwork {
  ...
  private addNode(x: number, y: number, z: number): number {
    ...
    console.log(`[NavNetwork] Added node ${node.index} at ${x},${y},${z}`);
    return node.index;
  }

  private connect(index1: number, index2: number) {
    const node1 = this.graph.getNode(index1);
    const node2 = this.graph.getNode(index2);
    if (!node1 || !node2) {
      console.error(`[NavNetwork] Invalid nodes: ${index1}, ${index2}`);
      return;
    }
    const dist = node1.position.distanceTo(node2.position);
    console.log(`[NavNetwork] Connecting ${index1}<->${index2} dist=${dist}`);

    // Create edges with valid costs
    const edge1 = new YUKA.Edge(index1, index2, dist);
    const edge2 = new YUKA.Edge(index2, index1, dist);
  ...
  private getClosestNode(position: THREE.Vector3): YUKA.NavNode | null {
    ...
    // Use standard API iteration
    const count = this.graph.getNodeCount();
    for (let i = 0; i < count; i++) {
      const node = this.graph.getNode(i);
      if (!node) continue;

      const distSq = position.distanceToSquared(
        node.position as unknown as THREE.Vector3,
      );
