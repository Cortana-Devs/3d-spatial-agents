...
export default function SocialWorkHub() {
  ...
  const {
    ...
    for (let i = 0; i < postCountX; i++) {
      for (let j = 0; j < postCountZ; j++) {
        const x =
          hubCenter.x -
          pergolaWidth / 2 +
          (i / (postCountX - 1)) * pergolaWidth;
        const z =
          hubCenter.z -
          pergolaDepth / 2 +
          (j / (postCountZ - 1)) * pergolaDepth;

        const matrix = new THREE.Matrix4();
        // Thinner posts (0.8 width instead of 2)
        matrix.compose(
          new THREE.Vector3(x, pergolaHeight / 2, z),
          new THREE.Quaternion(),
          new THREE.Vector3(0.8, pergolaHeight, 0.8),
        );
    ...
    for (let i = 0; i < slatCount; i++) {
      const z =
        hubCenter.z - pergolaDepth / 2 + (i / (slatCount - 1)) * pergolaDepth;
      const matrix = new THREE.Matrix4();
      matrix.compose(
        new THREE.Vector3(hubCenter.x, pergolaHeight, z),
        new THREE.Quaternion(),
        new THREE.Vector3(pergolaWidth + 4, 0.3, 0.3),
      );
      pergolas.push(matrix);
    }
    ...
    const createSeatingGroup = (
      ...
      for (let i = 0; i < sofaCount; i++) {
        ...
        for (let s = 0; s < 3; s++) {
          ...
          const cushionPos = pos
            .clone()
            .add(
              new THREE.Vector3(localX, seatHeight / 2, 0).applyQuaternion(rot),
            );
          ...
          mat.compose(
            cushionPos,
            rot,
            new THREE.Vector3(cushionWidth * 0.95, seatHeight, cushionDepth),
          );
          sofas.push(mat);
          ...
          const sitOffset = new THREE.Vector3(localX, 0, 1.4).applyQuaternion(
            rot,
          );
        ...
        for (let s = 0; s < 3; s++) {
          ...
          mat.compose(
            backPos,
            rot,
            new THREE.Vector3(cushionWidth * 0.95, backHeight, 1.5),
          );
          sofaBacks.push(mat);

          // Collision for Backrest: Reduced radius to 0.8 (Absolute Minimal)
          obs.push({
            position: backPos.clone().setY(hubCenter.y),
            radius: 0.8,
          });
        }
        ...
        const leftArmPos = pos
          .clone()
          .add(
            new THREE.Vector3(
              -sofaWidth / 2 + armWidth / 2,
              armHeight / 2,
              0,
            ).applyQuaternion(rot),
          );
        const rightArmPos = pos
          .clone()
          .add(
            new THREE.Vector3(
              sofaWidth / 2 - armWidth / 2,
        ...
        armL.compose(
          leftArmPos,
          rot,
          new THREE.Vector3(armWidth, armHeight, sofaDepth),
        );
        sofaArms.push(armL);
        ...
        obs.push({
          position: leftArmPos.clone().setY(hubCenter.y),
          radius: 0.8,
        });
        obs.push({
          position: rightArmPos.clone().setY(hubCenter.y),
          radius: 0.8,
        });
      }
      ...
      tableMatrix.compose(
        new THREE.Vector3(centerX, hubCenter.y + 0.4, centerZ),
        new THREE.Quaternion(),
        new THREE.Vector3(12, 0.4, 12),
      );
      tables.push(tableMatrix);
      obs.push({
        position: new THREE.Vector3(centerX, hubCenter.y, centerZ),
        radius: 6.0,
      });
    };
    ...
    return {
      pergolaMatrices: pergolas,
      sofaMatrices: sofas,
      sofaBackMatrices: sofaBacks,
      sofaArmMatrices: sofaArms,
      tableMatrices: tables,
      obstacles: obs,
      interactables: ints,
    };
  }, [hubCenter]);
  ...
  return (
    <group>
      ...
      <instancedMesh
        ref={pergolaRef}
        args={[undefined, undefined, pergolaMatrices.length]}
        castShadow
        receiveShadow
      >
      ...
      <instancedMesh
        ref={sofaRef}
        args={[undefined, undefined, sofaMatrices.length]}
        castShadow
        receiveShadow
      >
        <boxGeometry args={[1, 1, 1]} />
      ...
      <instancedMesh
        ref={sofaBackRef}
        args={[undefined, undefined, sofaBackMatrices.length]}
        castShadow
        receiveShadow
      >
        <boxGeometry args={[1, 1, 1]} />
      ...
      <instancedMesh
        ref={sofaArmRef}
        args={[undefined, undefined, sofaArmMatrices.length]}
        castShadow
        receiveShadow
      >
        <boxGeometry args={[1, 1, 1]} />
      ...
      <instancedMesh
        ref={tableRef}
        args={[undefined, undefined, tableMatrices.length]}
        castShadow
        receiveShadow
      >
        <boxGeometry args={[1, 1, 1]} />
      ...
      <mesh
        position={[hubCenter.x, hubCenter.y + 20, hubCenter.z - 40.5]}
        receiveShadow
        castShadow
      >
      ...
      <pointLight
        position={[hubCenter.x, hubCenter.y + 18, hubCenter.z]}
        intensity={1.5}
        distance={40}
        color="#ffaa55"
      />
      <pointLight
        position={[hubCenter.x - 15, hubCenter.y + 18, hubCenter.z]}
        intensity={1.0}
        distance={30}
        color="#ffaa55"
      />
